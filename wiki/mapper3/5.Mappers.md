...
方法：`List<T> selectByCondition(Object example);`<br>
说明：根据Condition条件进行查询<br><br>

接口：`SelectCountByConditionMapper<T>`<br>
方法：`int selectCountByCondition(Object example);`<br>
说明：根据Condition条件进行查询总数<br><br>

接口：`UpdateByConditionMapper<T>`<br>
方法：`int updateByCondition(@Param("record") T record, @Param("example") Object example);`<br>
说明：根据Condition条件更新实体`record`包含的属性，null值会被更新<br><br>

接口：`UpdateByConditionSelectiveMapper<T>`<br>
方法：`int updateByConditionSelective(@Param("record") T record, @Param("example") Object example);`<br>
说明：根据Condition条件更新实体`record`包含的不是null的属性值<br><br>

接口：`DeleteByConditionMapper<T>`<br>
方法：`int deleteByCondition(Object example);`<br>
